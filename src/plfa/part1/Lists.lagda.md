...
map-is-foldr {A} f = extensionality ext
  ...
  ext : (xs : List A) → map f xs ≡ foldr (λ x xs → f x ∷ xs) [] xs
  ext []       = refl
  ext (x ∷ xs) =
    begin
      map f (x ∷ xs)
...
```agda
fold-Tree : ∀ {A B C : Set} → (A → C) → (C → B → C → C) → Tree A B → C
fold-Tree leaf′ node′ (leaf x)            = leaf′ x
fold-Tree leaf′ node′ (node left y right) =
  node′ (fold-Tree leaf′ node′ left) y (fold-Tree leaf′ node′ right)
```
...
map-is-fold-Tree {A} {B} f g = extensionality ext
  ...
  ext :
    ...
    → map-Tree f g tree ≡
      fold-Tree
        (λ x → leaf (f x))
        (λ left y right → node left (g y) right)
        tree
  ext (leaf x)            = refl
  ext (node left y right) =
    begin
      map-Tree f g (node left y right)
    ≡⟨⟩
      node (map-Tree f g left) (g y) (map-Tree f g right)
    ≡⟨ cong₂ (λ left right → node left (g y) right) (ext left) (ext right) ⟩
      node
        (fold-Tree
          (λ x → leaf (f x))
          (λ left y right → node left (g y) right)
...
```agda
open import Data.Nat.Properties using (*-distribʳ-+; *-distribˡ-+; *-comm)
...
foldl-monoid-cons :
  ...
  → ∀ (x : A) (xs : List A) → foldl _⊗_ e (x ∷ xs) ≡ foldl _⊗_ x xs
foldl-monoid-cons _⊗_ e monoid-⊗ x xs =
  begin
    foldl _⊗_ e (x ∷ xs)
  ≡⟨⟩
    foldl _⊗_ (e ⊗ x) xs
  ≡⟨ cong (λ e → foldl _⊗_ e xs) (identityˡ monoid-⊗ x) ⟩
    foldl _⊗_ x xs
  ∎
...
foldl-monoid _⊗_ e ⊗-monoid (x ∷ xs) y =
  ...
  ≡⟨ cong (y ⊗_) (sym (foldl-monoid _⊗_ e ⊗-monoid xs x)) ⟩
    y ⊗ foldl _⊗_ x xs
  ≡⟨ cong (y ⊗_) (sym (foldl-monoid-cons _⊗_ e ⊗-monoid x xs)) ⟩
    y ⊗ foldl _⊗_ e (x ∷ xs)
  ∎
...
foldr-monoid-foldl {A} _⊗_ e monoid-⊗ = extensionality ext
  where
  ...
  ext (x ∷ xs) =
    ...
    ≡⟨ sym (foldl-monoid _⊗_ e monoid-⊗ xs x) ⟩
      foldl _⊗_ x xs
    ≡⟨ sym (foldl-monoid-cons _⊗_ e monoid-⊗ x xs)  ⟩
      foldl _⊗_ e (x ∷ xs)
    ∎
```
